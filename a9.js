...
function validate(inputID){
	...
	if(!str || !pattern.test(str) || str.length < 1 || str.length > 5){
		...
		document.getElementById(inputID+"Message").innerHTML='<span class="glyphicon glyphicon-remove-circle" aria-hidden="true" style="color:red"></span> '+strUP+" bad";
		return false;
	}
	input.setAttribute("class", "valid");
	document.getElementById(inputID+"Message").style.display = "inline-block";
	document.getElementById(inputID+"Message").innerHTML='<span class="glyphicon glyphicon-ok-circle" aria-hidden="true" style="color:#7F7"></span> '+strUP+" OK";
...
var timeoutID;
var currentSearch;
function delaySearch(inputID){
	var input=document.getElementById(inputID).value;
	setTimeout(function(){console.log(input);}, 1000);
	clearInterval(timeoutID);
	timeoutID=setTimeout(function(){listUsers(input);}, 1000);
}
function listUsers(input){
	...
	xhr.addEventListener("load", function(){
		...
		if(!xhr.response[1]){
			...
			for(var i = 1; i < xhr.response.length; i++){
				...
				li.innerHTML='<button type="button" class="deleteButton" onclick=\'purifyAccount("'+xhr.response[i].Name+'")\'><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>' + xhr.response[i].Name;
				list.appendChild(li);
	...
	xhr.open("GET", "//cse.taylor.edu/~cos143/users.php?Name=" + input);
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.send();
}
function purifyAccount(username){
	console.log("attempt delete..." + username)
	var xhr = new XMLHttpRequest();
	var params = "Name="+username;
	xhr.responseType = "json";
	xhr.addEventListener("load", function(){
		document.getElementById("createMessage").innerHTML=xhr.response[0].message;
		console.log(xhr.response[0]);
		listUsers(currentSearch);
		alert(xhr.response[0].message);
	});
	xhr.open("POST", "//cse.taylor.edu/~cos143/users.php")
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.send("method=delete&"+params);
